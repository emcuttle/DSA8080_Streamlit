...
with st.sidebar.expander("What are cluster hotspots?"):
    st.write("Hotspots are statistically significant clusters of predicted damage.")
...
try:
    ...
    gdf["outcome"] = "TN"
    gdf.loc[(gdf["label"] == 1) & (gdf["prediction_class"] == 1), "outcome"] = "TP"
    gdf.loc[(gdf["label"] == 1) & (gdf["prediction_class"] == 0), "outcome"] = "FN"
    gdf.loc[(gdf["label"] == 0) & (gdf["prediction_class"] == 1), "outcome"] = "FP"
    gdf.loc[(gdf["label"] == 0) & (gdf["prediction_class"] == 0), "outcome"] = "TN"
    ...
    if enable_hotspots:
        ...
        <div style="display: flex; gap: 20px; font-weight: bold;">
          <div style="width:20px;height:20px;background:red;"></div> Hotspot
          <div style="width:20px;height:20px;background:blue;"></div> Coldspot
          <div style="width:20px;height:20px;background:gray;"></div> Not significant
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style="display: flex; gap: 20px; font-weight: bold;">
          <div style="width:20px;height:20px;background:red;"></div> TP
          <div style="width:20px;height:20px;background:yellow;"></div> FN (missed damage)
          <div style="width:20px;height:20px;background:blue;"></div> FP
          <div style="width:20px;height:20px;background:gray;"></div> TN
        </div>
        """, unsafe_allow_html=True)
    ...
    if enable_hotspots:
        ...
        gdf["fill_color"] = gdf["outcome"].map(outcome_colors)
